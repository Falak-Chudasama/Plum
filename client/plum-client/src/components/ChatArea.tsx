...
function Response({ responseObj }: { responseObj: ResponseType }) {
    ...
    const handleSendMailClick = async () => {
        ...
        setEmailSent(true);
        try {
            await apis.sendMail(responseObj.craftedMail);
        } catch (err) {
            setEmailSent(false);
        }
    }
    ...
    const handleDraftMailClick = async () => {
        if (emailDrafted) return;
        setEmailDrafted(true);
        try {
            await apis.draftMail(responseObj.craftedMail);
        } catch (err) {
    ...
    function DraftMailBt() {
        return (
            <button onClick={() => handleDraftMailClick()} className={`bg-plum-primary flex duration-300 items-center justify-center ${emailDrafted ? 'cursor-not-allowed opacity-75' : 'cursor-pointer'} rounded-full px-4 py-1 hover:px-5 duration-200 font-medium`}>
    ...
    return (
        <div className={`pr-8 text-xl duration-300 font-light ${isCurrentResponse ? '' : ''}`}>
            ...
            {responseObj.query?.result && (
                <div className="max-h-[30vh] relative pt-10 shadow-plum-surface-xs w-fit grid gap-2 rounded-xl bg-plum-bg-bold p-3 overflow-y-auto">
                    <div className="top-2 left-4 absolute flex items-center gap-1 bg-plum-primary-dark shadow-plum-secondary-sm px-2 rounded-full">
                        <span className="h-2.5 w-2.5 rounded-full bg-plum-bg"></span>
            ...
            {(
                responseObj.craftedMail ? (
                    <div className="mt-3 py-2 px-3 pt-10 relative text-[15px] rounded-xl bg-plum-bg-bold text-plum-secondary max-w-[45vw] duration-300">
                        <div className="top-2 left-3 absolute flex items-center gap-1 bg-plum-primary-dark shadow-plum-secondary-sm px-2 rounded-full">
                            <span className="h-2.5 w-2.5 rounded-full bg-plum-bg"></span>
                            <p className="text-white text-[15px] font-medium">
                                Crafted Email
                            </p>
                        ...
                        <div className="flex gap-x-1 mt-4 justify-end m-2">
                            <DraftMailBt />
                            <SendMailBtn />
                        </div>
                    </div>
...
function Prompt({ promptObj }: { promptObj: UserPromptType }) {
    ...
    return (
        <div className="px-4 py-1 mb-2 text-lg text-plum-bg rounded-2xl bg-plum-primary max-w-[40vw] break-words whitespace-pre-wrap">
            {promptObj.prompt}
        </div>
    );
}
        ...
        <div className="items-end duration-300 text-[18px]">
            {
                chatArr.map((msg, idx) => {
                    if ('prompt' in msg) {
                        ...
                        return (
                            <div id={msg.response + msg.createdAt + idx} className="mb-8">
